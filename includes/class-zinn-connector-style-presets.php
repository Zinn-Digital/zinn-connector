 ...
 */
final class Zinn_Connector_Style_Presets {
	...
	public static function fields( string $id ): array {
		...
		$fields = array(
			array(
				...
				'type'        => 'radio',
				'label'       => __( 'Style preset', 'zinn-connector' ),
				'description' => __( 'A starting point. Anything below overrides it.', 'zinn-connector' ),
				'choices'     => $choices,
			...
			array(
				...
				'type'        => 'toggle',
				'label'       => __( 'Use my own colours and sizes', 'zinn-connector' ),
				'description' => __( 'Leave this off and the preset is used exactly as it is.', 'zinn-connector' ),
				'default'     => false,
			),
		);
	...
	public static function resolve( string $id ): array {
		...
		$chosen  = (string) Zinn_Connector_Admin_UI::get( self::preset_key( $id ), (string) array_key_first( $presets ) );
		...
		if ( ! empty( Zinn_Connector_Admin_UI::get( self::custom_key( $id ), false ) ) ) {
			foreach ( (array) $component['tokens'] as $token => $spec ) {
				$value = (string) Zinn_Connector_Admin_UI::get( self::token_key( $id, (string) $token ), '' );
				if ( '' !== $value ) {
	...
	public static function enqueue_front_end_css(): void {
		...
		$handle = 'zinn-zinn-connector-tokens';
