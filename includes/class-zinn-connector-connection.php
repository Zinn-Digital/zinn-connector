 ...
 */
final class Zinn_Connector_Connection {
	...
	public static function label( string $state ): string {
		switch ( $state ) {
			case 'connected':
				return __( 'Connected', 'zinn-connector' );
			case 'connecting':
				return __( 'Connecting…', 'zinn-connector' );
			case 'degraded':
				return __( 'Connected, but something needs attention', 'zinn-connector' );
			case 'disconnected':
				return __( 'Not connected', 'zinn-connector' );
			case 'standalone':
				return __( 'Running on your own account', 'zinn-connector' );
			default:
				return __( 'We could not check this', 'zinn-connector' );
		}
	}
	...
	public static function normalise( array $status ): array {
		...
		if ( '' === $reason && ! in_array( $state, array( 'connected', 'standalone' ), true ) ) {
			$reason = __( 'This plugin did not say why. Send a diagnostics report and we will look.', 'zinn-connector' );
		}
	...
	public static function render_card( array $status ): void {
		...
		?>
		<section class="zinn-connection zinn-connection--<?php echo esc_attr( sanitize_html_class( $state ) ); ?>" aria-label="<?php esc_attr_e( 'Connection status', 'zinn-connector' ); ?>">
				...
				<?php if ( (int) $status['checked_at'] > 0 ) : ?>
					<p class="zinn-connection__checked">
						...
						printf(
							...
							esc_html__( 'Last checked %s ago.', 'zinn-connector' ),
	...
	private static function render_action( array $action ): void {
		...
		>
			...
			<?php if ( $external ) : ?>
				<span class="screen-reader-text"><?php esc_html_e( '(opens in a new tab)', 'zinn-connector' ); ?></span>
			<?php endif; ?>
		</a>
		<?php
	}
