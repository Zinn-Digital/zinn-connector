 ...
 */
final class Zinn_Connector_Streaming_Upload {
	...
	public static function put_file( string $url, string $path, string $content_type ) {
		...
		if ( null === $size ) {
			return new WP_Error(
				...
				__( 'That file could not be read from this server, so nothing was uploaded.', 'zinn-connector' )
			);
		}
	...
	private static function put_streamed( string $url, string $path, string $content_type, int $size ) {
		...
		if ( ! $attached ) {
			...
			return new WP_Error(
				...
				__( 'This site could not stream the upload, so nothing was sent.', 'zinn-connector' )
			);
		}
		...
		if ( $size > $cap ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'This file is %1$s and this site\'s PHP has no cURL, so the largest it can upload is %2$s.', 'zinn-connector' ),
					size_format( $size ),
					size_format( $cap )
				)
			);
	...
	private static function settle( $response ) {
		if ( is_wp_error( $response ) ) {
			...
			return new WP_Error(
				'zinn_upload_failed',
				__( 'The upload could not be completed.', 'zinn-connector' )
			);
		}
		...
		if ( $status < 200 || $status > 299 ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Storage refused the upload (HTTP %d).', 'zinn-connector' ),
					$status
				)
			);
		}
